...
SENT_ALERTS_FILE = "/home/tsalisbury0/meshtastic_test/sent_alerts.pkl"  # Updated path
                        ...
                        if not SAME_CODES.isdisjoint(same_codes) and alert_id not in sent_alerts:
                            ...
                            for code in same_codes:
                                if code in SAME_CODE_MAP:
                                    ...
                                    message = f"MeshSTL Alert:\n⚠️ {title} for {county} until {expires_str}"
...
def send_meshtastic_message(messages):
    for message in messages:
        try:
            subprocess.run(["meshtastic", "--sendtext", message, "--ch-index", "3", "--dest", "^all"], check=True)
